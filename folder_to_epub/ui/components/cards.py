"""Modular card widgets for the dashboard."""
...
class CoverCard(BaseCard):
    """Card 2: Cover image preview & actions."""

    def __init__(self, parent: ctk.CTkFrame, on_choose_cover: Callable[[], None], on_reset_cover: Callable[[], None], **kwargs):
        ...
        self.thumb_box = ctk.CTkFrame(preview_row, width=80, height=110, corner_radius=8, fg_color=("gray85", "#262b35"))
        self.thumb_box.pack(side="left", padx=(0, 14))
        self.thumb_box.pack_propagate(False)

        self.thumb_label = ctk.CTkLabel(self.thumb_box, text="No\nCover", font=ctk.CTkFont(size=11), text_color="gray50")
        self.thumb_label.place(relx=0.5, rely=0.5, anchor="center")

        info_col = ctk.CTkFrame(preview_row, fg_color="transparent")
        info_col.pack(side="left", fill="both", expand=True)

        self.status_label = ctk.CTkLabel(info_col, text="No cover detected", font=ctk.CTkFont(size=12), anchor="w")
        self.status_label.pack(fill="x", pady=(0, 6))

        btn_row = ctk.CTkFrame(info_col, fg_color="transparent")
        btn_row.pack(fill="x")

        self.choose_btn = ctk.CTkButton(btn_row, text="Choose...", width=75, height=28, font=ctk.CTkFont(size=11), command=on_choose_cover)
...
class MetadataCard(BaseCard):
    """Card 3: Title, author, and language inputs."""

    def __init__(self, parent: ctk.CTkFrame, title_var: ctk.StringVar, author_var: ctk.StringVar, lang_var: ctk.StringVar, lang_options: tuple, **kwargs):
        ...
        self.lang_menu = ctk.CTkOptionMenu(grid, values=[opt[1] for opt in lang_options], variable=lang_var, height=28)
        self.lang_menu.grid(row=2, column=1, sticky="ew", padx=(8, 0), pady=4)


class LayoutCard(BaseCard):
    """Card 4: Manga Fixed-Layout and RTL switches."""

    def __init__(self, parent: ctk.CTkFrame, manga_var: ctk.BooleanVar, rtl_var: ctk.BooleanVar, on_manga_toggle: Callable[[], None], **kwargs):
...
class DestinationCard(BaseCard):
    """Card 5: Destination path and open actions."""

    def __init__(self, parent: ctk.CTkFrame, path_var: ctk.StringVar, on_browse_output: Callable[[], None], on_open_epub: Callable[[], None], on_open_folder: Callable[[], None], **kwargs):
        super().__init__(parent, "💾 5. EPUB Destination", "Output file or destination folder", **kwargs)

        row = ctk.CTkFrame(self.body, fg_color="transparent")
        row.pack(fill="x", pady=(4, 8))

        self.entry = ctk.CTkEntry(row, textvariable=path_var, placeholder_text="Output location...", height=34)
        self.entry.pack(side="left", fill="x", expand=True, padx=(0, 8))

        self.browse_btn = ctk.CTkButton(row, text="Browse...", width=85, height=34, command=on_browse_output)
        self.browse_btn.pack(side="right")
...
class StatusCard(BaseCard):
    """Card 6: Status preview with quick link to Activity Log."""

    def __init__(self, parent: ctk.CTkFrame, on_view_logs: Callable[[], None], **kwargs):
